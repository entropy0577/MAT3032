...
#set text(font: "New Computer Modern", size: 5.5pt, weight: "regular")
#set par(justify: true, leading: 0.35em, spacing: 0.4em)
#set heading(numbering: none)
#show heading.where(level: 1): set text(size: 7pt, weight: "bold")
#show heading.where(level: 2): set text(size: 6pt, weight: "bold")
#show heading.where(level: 3): set text(size: 5.5pt, weight: "bold")
#show heading: set block(above: 0.3em, below: 0.15em)
#set enum(indent: 0pt, body-indent: 2pt, spacing: 0.3em)
#set list(indent: 0pt, body-indent: 2pt, spacing: 0.3em)
#show: columns.with(5, gutter: 4pt)
...
*Group families:*
- *Cyclic* $C_n = angle.l g : g^n = e angle.r cong (ZZ_n, +_n)$; $phi(n)$ generators; unique subgp of order $d$ for each $d | n$
...
- *Direct products*: $C_m times C_n cong C_(m n)$ iff $gcd(m,n) = 1$

== §1.2 Generators & Presentations

#defn[Generating Set][$angle.l S angle.r$ = smallest subgroup containing $S$.]
#defn[Presentation][$angle.l S : R angle.r$: generators and defining relations.]
Examples: $C_n = angle.l g : g^n = e angle.r$; Klein $V = angle.l a,b : a^2 = b^2 = e, a b a = b angle.r$; $D_n = angle.l sigma, tau : sigma^n = tau^2 = iota, sigma tau sigma = tau angle.r$.

*Groups of order 8:* 5 total: $C_8$, $C_4 times C_2$, $C_2^3$ (abelian); $D_4$, $Q_4$ (non-abelian).
_Derivation:_ Non-abelian of order 8: all non-id elements have order 2 or 4; at least one has order 4 (else abelian). Pick $a$ of order 4, $b in.not angle.l a angle.r$. Then $G = {e, a, a^2, a^3, b, a b, a^2 b, a^3 b}$. Since $b^2 in angle.l a angle.r$: $b^2 = e => D_4$; $b^2 = a^2 => Q_4$.

#defn[Dicyclic][$Q_n$: order $2n$, $angle.l a,b : a^n = e, b^2 = a^(n\/2), a b a = b angle.r$.]
...
#thm[Prop 1.4][$|H J| = (|H| dot |J|) / |H sect J|$.]
#thm[Prop 1.5 (Internal Direct Product)][If $G = H J$, $H sect J = {e}$: (i) unique $h j$ expression; (ii) $H, J lt.tri.eq G$ iff $h j = j h$ $forall h,j$; then $G cong H times J$.]
#prf[Uniqueness: $h j = h' j' => h'^(-1) h = j' j^(-1) in H sect J = {e}$. Commutativity$<=>$normality: if both normal, $h j h^(-1) j^(-1) in H sect J = {e}$.]

#defn[Quotient Group][$G\/H$ when $H lt.tri.eq G$: cosets $a H$ form group, $(a H)(b H) = a b H$.]
#thm[1st Isomorphism Thm][$phi: G_1 -> G_2$ homom. $=>$ $ker phi lt.tri.eq G_1$ and $G_1 \/ ker phi cong phi(G_1)$.]
#thm[Prop 1.7 (Correspondence Thm)][Subgroups of $G\/N$ are $H\/N$ for $H lt.eq G$ with $N lt.eq H$. $H\/N lt.tri.eq G\/N$ iff $N lt.tri.eq H lt.tri.eq G$, and $(G\/N)\/(H\/N) cong G\/H$ (*3rd Isom. Thm*).]
#thm[2nd Isomorphism Thm][$H\/(H sect N) cong H N\/N$.]
#thm[Prop 1.8][If $G\/N$ cyclic, then $G$ has a subgroup $cong G\/N$.]
#prf[If $g N$ generates $G\/N$ with order $m$, $g$ has order $r = k m$. Then $angle.l g^k angle.r cong G\/N$.]

== §1.4 Finite Abelian Groups

#thm[Fundamental Theorem][Every finite abelian group $cong$ direct product of cyclics:
(1) *Primary:* $C_(p_1^(n_1)) times dots times C_(p_r^(n_r))$;
(2) *Invariant factor:* $C_(q_1) times dots times C_(q_s)$ with $q_i | q_(i+1)$.]
#prf[Induction on min. generating set size. Key: if $angle.l g_1 angle.r sect H eq.not {e}$, construct element of smaller order in a $k$-element gen. set, contradicting minimality.]
...
#thm[Prop 1.11][$"Aut"(C_n) cong U_n$, $|"Aut"(C_n)| = phi(n)$. Automorphisms: $psi_m : g |-> g^m$ for $gcd(m,n) = 1$.]
#defn[Inner Auts][$gamma_a : x |-> a x a^(-1)$; $"Inn"(G) lt.tri.eq "Aut"(G)$; $G\/Z(G) cong "Inn"(G)$.]
#defn[Semi-Direct Product][$H lt.times J$: $G = H J$, $H sect J = {e}$, $H lt.tri.eq G$ (but $J$ need not be normal). $J$ acts on $H$ by conjugation.]

*Groups of order 12:* $H cong C_3$, $J cong C_4$, $H lt.tri.eq G$: conjugation by $j$ maps $h -> h$ (gives $C_12$) or $h -> h^2$ (non-abelian).
...
#thm[Prop 2.8][Every group of order $p^2$ is abelian ($cong C_(p^2)$ or $C_p times C_p$).]
...
#thm[Prop 2.19][$p$-group on finite $X$: $|"Fix"(G)| equiv |X| pmod(p)$. So $Z(G) eq.not {e}$.]
#thm[Prop 2.20][$p$-subgroup $H$: $[N_G(H):H] equiv [G:H] pmod(p)$.]
#thm[Sylow Theorems (Prop 2.21)][$|G| = p^k m$, $p divides.not m$:
(i) $exists$ $p$-Sylow (order $p^k$); (ii) every $p$-subgroup $subset.eq$ some $p$-Sylow; (iii) all $p$-Sylows conjugate; (iv) $w_p equiv 1 pmod(p)$ and $w_p | m$; (v) $[G:N_G(P)] = w_p$.]
#prf[(i)+(ii): Start with $|H| = p$ (Cauchy). Prop 2.20 gives $p | [N_G(H):H]$ for proper $p$-subgp, so $exists$ larger one by Correspondence Thm.
(iii): $P'$ acts on $G\/P$; $|"Fix"(P')| equiv m pmod(p) eq.not 0$, so $exists b P$ fixed by $P'$, giving $P' subset.eq b P b^(-1)$; equal order $=> P' = b P b^(-1)$.
(iv): $w_p = [G:N_G(P)]$; $q = [N_G(P):P]$; $q w_p = m$ so $w_p | m$. By 2.20, $m = q + k p$ so $w_p = 1 + k p\/q$; $p divides.not q$ gives $w_p equiv 1 pmod(p)$.]

#thm[Prop 2.22][(i) Unique $p$-Sylow $<==>$ normal. (ii) Abelian $=>$ unique $p$-Sylows. (iii) All unique $=>$ $G cong$ direct product of Sylows.]
...
*Key Sylow examples:*
Order 133 ($= 7 times 19$): $w_19 = 1$, $w_7 = 1$, both normal, $G cong C_133$.
Order 45 ($= 3^2 times 5$): $w_5 = 1$, $w_3 = 1$; groups: $C_45$, $C_3 times C_15$.
Order 455 ($= 5 times 7 times 13$): $w_13 = 1$, $w_7 = 1$, $w_5 = 1$; $G cong C_455$.
...
#thm[Prop 3.6][If $R = I + J$, $I sect J = {0}$, then $R cong I times J$.]
#thm[Prop 3.7][Central idempotent $e$: $R = e R plus.circle (1-e)R$.]

*Key results:* $e R e$ is subring with unity $e$; $e R e = {a : e a = a e = a}$. $e + (1-e)r e$ is idempotent. $1 - 2e$ is self-inverse unit.
$ZZ_18$: $9^2 = 9$; $ZZ_18 = 9 ZZ_18 plus.circle 10 ZZ_18$.
$ZZ_30$ idempotents: ${0,1,6,10,15,16,21,25}$; decomp: $ZZ_30 = 6 ZZ_30 plus.circle 10 ZZ_30 plus.circle 15 ZZ_30$.
...
#thm[Prop 3.9][$e = 1/(|G|) sum_(x in G) x$ is a central primitive idempotent. $K G = e K G plus.circle (1-e) K G$; $dim_K(e K G) = 1$.]
...
*If $|H|^2 > |G|$ and $|J|^2 > |G|$: $H sect J eq.not {e}$.*
...
*$D_6 cong D_3 times C_2$.*
*$A_4$ has no subgroup of order 6.*
*\#conj. classes = $1/(|G|) sum |C_G(g)|$.*

== Exam Strategies

*Sylow analysis:* Write $|G| = p^k m$; list divisors of $m$ that are $equiv 1 pmod(p)$; use element counting to eliminate cases.
...
*Ring decomposition:* Find central idempotents $e_1, dots, e_k$ with $sum e_i = 1$, pairwise orthogonal; $R = e_1 R plus.circle dots plus.circle e_k R$.
