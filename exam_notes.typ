...
#defn[Element Order][$o(a)$ is the smallest $k in NN^+$ s.t. $a^k = e$. Note: By Lagrange, $o(a) | |G|$.]
#defn[Homomorphism][Map $phi: G_1 -> G_2$ preserving op. *Kernel*: $ker phi = {g : phi(g) = e}$. *Image*: $phi(G_1)$. Injective iff $ker phi = {e}$.]

*Group Families:* *Cyclic* $C_n = chevron.l g : g^n = e chevron.r tilde.eq (ZZ_n, +_n)$, $phi(n)$ generators, unique subgp of order $d | n$. *Units* $U_n$: units of $(ZZ_n, times_n)$, order $phi(n)$. *$"GL"(n,R)$, $"SL"(n,R)$*: general/special linear. *$S_n$*: symmetric gp, order $n!$. *$A_n$*: alternating, order $n! \/ 2$. *$D_n$*: dihedral, regular $n$-gon syms, order $2n$. *Direct products*: $C_m times C_n tilde.eq C_(m n)$ iff $gcd(m,n) = 1$.
...
*Examples:* $C_n = chevron.l g | g^n = e chevron.r$; Klein Four $V = chevron.l a,b | a^2 = b^2 = e, a b a = b chevron.r$; $D_n = chevron.l sigma, tau | sigma^n = tau^2 = iota, sigma tau sigma = tau chevron.r$.

*Groups of order 8:* 5 total: $C_8$, $C_4 times C_2$, $C_2^3$ (abelian); $D_4$, $Q_4$ (non-abelian).
_Derivation:_ For non-abelian order 8, non-identity elements have order 2 or 4. Pick $a$ of order 4, $b in.not chevron.l a chevron.r$. $G = {e, a, a^2, a^3, b, a b, a^2 b, a^3 b}$. $b^2 in chevron.l a chevron.r$ commutes w/ $b$, so $b^2 = e$ or $a^2$. We must also establish $b a b^(-1) = a^(-1)$ (or $b a = a^3 b$) to ensure non-abelian. If $b^2 = e => D_4$. If $b^2 = a^2 => Q_4$.

#defn[Dicyclic Group][$Q_n$: order $2n$, $chevron.l a,b | a^n = e, b^2 = a^(n\/2), a b a = b chevron.r$.]

== Cosets, Products, Quotients

#defn[Cosets][Left coset $a H = {a h : h in H}$, right $H a$. Cosets partition $G$.]
#defn[Normal Subgroup][$H lt.tri.eq G$ if $a H = H a$ $forall a in G$. Equiv: $a H a^(-1) = H$ $forall a$. Index 2 is normal. Kernels are normal.]
#thm[Normality via Generators][For finite $G$, $H lt.tri.eq G$ iff $g h g^(-1) in H$ $forall g$ in gen. set for $G$, $forall h$ in gen. set for $H$.]
...
#thm[Internal Direct Product][If $G = H J$ and $H inter J = {e}$: Every elt has unique $h j$. $H, J lt.tri.eq G$ iff $h j = j h$ $forall h, j$. In this case, $G tilde.eq H times J$.]
#prf[Uniqueness: $h j = h' j' => (h')^(-1) h = j' j^(-1) in H inter J = {e}$. Commute $<=>$ normal: Commutator $h j h^(-1) j^(-1) in H inter J = {e} => h j = j h$.]

#defn[Quotient Group][$G\/H$ exists when $H lt.tri.eq G$. Cosets $a H$ form a group $(a H)(b H) = a b H$.]
#thm[1st Isomorphism Theorem][If $phi: G_1 -> G_2$ is a homom., then $ker phi lt.tri.eq G_1$ and $G_1 \/ ker phi tilde.eq phi(G_1)$.]
#thm[Correspondence Theorem][Subgroups of $G\/N$ are $H\/N$ for $H lt.eq G$ containing $N$. Assuming $N lt.tri.eq G$, $H\/N lt.tri.eq G\/N <=> H lt.tri.eq G$, and $(G\/N)\/(H\/N) tilde.eq G\/H$ (*3rd Isom. Thm*).]
...
#prf[If $g N$ generates $G\/N$ with order $m$, $g$ has order $r = k m$. Subgroup $chevron.l g^k chevron.r tilde.eq G\/N$.]
...
*Examples:* Order 54 abelian gps: $C_54$, $C_3 times C_18$, $C_3^2 times C_6$. \# abelian groups of order $p^k$ is the \# of partitions of $k$.
*Element order:* $o(g_1, dots, g_s) = "lcm"(o(g_1), dots, o(g_s))$. 
*Generating Sets:* Abelian group generated by elements w/ coprime orders has a gen. set formed by products of prime powers.

== Automorphisms & Semi-Direct Products

#thm[Automorphism Group of $C_n$][$"Aut"(C_n) tilde.eq U_n$, $|"Aut"(C_n)| = phi(n)$. Auts are $psi_m : g |-> g^m$ for $gcd(m,n) = 1$.]
#defn[Inner Automorphisms][$gamma_a : x |-> a x a^(-1)$. $"Inn"(G) lt.tri.eq "Aut"(G)$, and $G\/Z(G) tilde.eq "Inn"(G)$.]
#defn[Semi-Direct Product][$H times.r J$ occurs when $G = H J$, $H inter J = {e}$, $H lt.tri.eq G$. $J$ acts on $H$ by conjugation.]

*Groups of order 12:* Let $H tilde.eq C_3, J tilde.eq C_4, H lt.tri.eq G$. Conjugation by $j in J$ maps $h |-> h$ (yielding abelian $C_12$) or $h |-> h^2$ (yielding non-abelian).
*Characteristic subgroups:* Transitive property ($K "char" H "char" G => K "char" G$). If $N lt.tri.eq G$, $G\/N$ cyclic, and $|N|, |G\/N|$ coprime, then $G = N times.r Q$ (where $Q tilde.eq G\/N$).
*Homom vs Aut:* $phi(a)=a^2$ is a homom iff $G$ is abelian. $"Aut"(ZZ) tilde.eq C_2$.
...
#defn[Orbit and Stabiliser][*Orbit*: $O(x) = {g(x) : g in G}$. *Stabiliser*: $G_x = {g : g(x) = x} lt.eq G$. *Fixed pts*: $"Fix"(g) = {x : g(x) = x}$.]
#thm[Orbit-Stabiliser Theorem][Bijection $g(x) <-> g G_x$. For finite $G$: $|O(x)| dot |G_x| = |G|$.]
#prf[$h(x) = g(x) <=> g^(-1)h in G_x <=> h in g G_x$.]
#thm[Orbit Decomposition][$|X| = |"Fix"(G)| + sum_(i=1)^n [G:G_(x_i)]$ (sum over non-trivial orbit reps).]
#thm[Action Homomorphisms][Action on set size $m =>$ homom $G -> S_m$ w/ kernel $tilde(G)_X$. Subgroup index $m =>$ homom $G -> S_m$ acting on left cosets, kernel $subset.eq$ subgroup.]
...
#defn[Centraliser and Centre][*Centraliser*: $C_G(x) = {g : g x = x g}$. *Centre*: $Z(G) = {g : g x = x g forall x} lt.tri.eq G$.]
...
*Conjugacy in $A_n$:* $S_n$-class stays single in $A_n$ unless centraliser $subset.eq A_n$, then it splits in two.
*$A_5$ is simple:* Class sizes are 1, 12, 12, 15, 20. No non-trivial union divides 60.
*Parity:* $k$-cycle is even iff $k$ is odd. $sigma$ is even iff $n - "cyc"(sigma)$ is even. 
*Cycle Counting:* \# $k$-cycles in $S_n$ is $binom(n,k)(k-1)!$.
*Conjugacy in $A_4$:* $(1 space 2 space 3)$ and $(1 space 3 space 2)$ NOT conjugate in $A_4$ (conjugator is odd).
*Normal Subgroups of $S_4$:* ${e}$, $V = {e, (12)(34), (13)(24), (14)(23)}$, $A_4$, $S_4$.

== Burnside's Formula

#thm[Burnside's Lemma][\# orbits is $1/(|G|) sum_(g in G) |"Fix"(g)|$.]
#prf[Count pairs $g(x) = x$: $sum |"Fix"(g)| = sum |G_x|$. By orbit-stab, $sum_(x in O) |G_x| = |G|$ per orbit, contributing exactly $|G|$ to total.]
#thm[Colouring Formula][$q$ colours, symmetry $G subset.eq S_n$: \# colourings is $1/(|G|) sum_(sigma in G) q^("cyc"(sigma))$.]
...
#thm[$p$-Groups on Finite Sets][$p$-group acting on finite $X$, $|"Fix"(G)| equiv |X| (mod p)$. Letting $G$ act on itself, $Z(G) eq.not {e}$.]
...
#prf[(i)+(ii): Cauchy gives order $p$. Normaliser thm gives $p | [N_G(H):H]$, Corresp Thm guarantees larger until $p^k$. (iii): $P'$ acts on $G\/P$. $|"Fix"(P')| equiv m eq.not 0 (mod p)$, coset $b P$ fixed, so $P' subset.eq b P b^(-1)$. (iv): $w_p = [G:N_G(P)]$. Let $q = [N_G(P):P]$. $q w_p = m => w_p | m$. Normaliser thm $m = q + k p => w_p equiv 1 (mod p)$.]
...
#thm[Coprime Automorphisms][If $H lt.tri.eq G$ cyclic, $phi(|H|)$ coprime to $|J|$, then $h j = j h$ $forall h, j$. If $phi(|H|)$ coprime to $|G|$ then $H subset.eq Z(G)$.]
#prf[Action defines homom $theta: J -> "Aut"(H)$. $|"Aut"(H)| = phi(|H|)$ coprime to $|J| => |theta(J)| = 1$.]

*Key Sylow Examples:* *Order 133 ($7 times 19$):* $w_19 = 1, w_7 = 1$. Both normal, $G tilde.eq C_133$. *Order 45 ($3^2 times 5$):* $w_5 = 1, w_3 = 1$. Groups: $C_45, C_3 times C_15$. *Order 455 ($5 times 7 times 13$):* $w_13 = 1, w_7 = 1 => C_91 tilde.eq P_7 P_13 lt.tri.eq G$. $G tilde.eq C_91 times.r C_5$. $"Aut"(C_91) = 72$ coprime to 5. Semidirect product must be direct (abelian), forcing $w_5 = 1$. $G tilde.eq C_455$.
...
#prf[Embed in $S_(2m)$ via Cayley. Order 2 elt has no fixed pts, product of $m$ transpositions. $m$ odd $\to$ odd perm. $G subset.not A_(2m)$, gives normal subgp index 2.]
...
*Non-simplicity Tests:* *Sylow Index:* $|G| = p^k m$ ($1 lt.eq m < p$, not $k=m=1$), $p$-Sylow has index $m < p$. Homom into $S_m$, but $|G|$ cannot divide $m!$. *Order 36:* $w_3 in {1,4}$. If 4, action on Sylows gives index 4 subgp. Bound: $36 | 4! = 24$ (contradiction). *Order $p^2 q^2$ ($p > q$ odd):* $w_p in {1, q, q^2}$. $w_p eq.not q$. If $q^2$, $p | q^2 - 1 = (q-1)(q+1) => p = q+1$ (impossible). Thus $w_p = 1$. *Exactly 2-elt class:* implies not simple (centraliser index 2 is normal).

= Rings, Fields, Algebras

== Rings & Fields

#defn[Ring][$(R, +, times)$ abelian gp under $+$, semigroup under $times$, distributive.]
#defn[Subring Test][Non-empty $S$ is subring if $a - b in S$ and $a b in S$ $forall a,b$.]
#defn[Quaternions $HH$][$a + b i + c j + d k$, $i^2 = j^2 = k^2 = -1, i j = k, j k = i, k i = j$. Anti-commutative.]
...
#defn[Ideals][$I$ is additive subgroup of $R$. *Left*: $r a in I$. *Right*: $a r in I$. *Simple* ring: no non-trivial proper two-sided ideals.]
...
#thm[Idempotent Properties][$e^2 = e => (1-e)^2 = 1-e$, $e(1-e) = 0$.]
...
*Examples:* $ZZ_18$: $9^2 = 9 => ZZ_18 = 9 ZZ_18 plus.o 10 ZZ_18$. $ZZ_30$ idempotents: ${0,1,6,10,15,16,21,25}$, decomp: $ZZ_30 = 6 ZZ_30 plus.o 10 ZZ_30 plus.o 15 ZZ_30$.

== Algebras

#defn[$K$-Algebra][Vector space over $K$ w/ bilinear mult. Associative, has unity.]
...
#defn[Group Algebra $K G$][Finite group $G = {x_1, dots, x_n}$, field $K$, $K G = {sum a_i x_i | a_i in K}$, dimension $n$.]
...
#prf[_Idempotent:_ $x_i$ permutes $G => x_i e = e$. $e^2 = 1/n(x_1 e + dots + x_n e) = e$. _Primitive:_ $e = f_1 + f_2$, $f_1 f_2 = 0 => e f_i = f_i => f_i in e K G = "span"{e} => f_i = lambda_i e$. $lambda_1+lambda_2=1, lambda_1 lambda_2=0$, cannot decompose.]

*Group Algebra Properties:* *$K C_n$:* $g - 1$ is zero divisor: $(g-1)(1 + g + dots + g^(n-1)) = 0$. Never division algebra. *$F_2 C_3$:* 8 elts. $(1+g+g^2)$ idempotent, not unit. Non-zero elements under mult not a group. *$CC C_3$:* $f = 1/3(1 + omega g + omega^2 g^2)$ ($omega = e^(2 pi i\/3)$) is idempotent orthogonal to $e = 1/3(1+g+g^2)$.

== Key Techniques

*Order counting:* Elts of order $k$ come in groups of $phi(k)$ (from cyclic subgps). Non-cyclic order 55 has $2r$ elts order 5, $5s$ order 11, $2r + 5s = 54$. *Order 2 elements:* Even order group has odd \# of order 2 elts (non-self-inverse pair up). *Subgroup Intersections:* If $|H|^2 > |G|$ and $|J|^2 > |G|$, then $H inter J eq.not {e}$ since $|H inter J| = (|H||J|)\/|H J| gt.eq (|H||J|)\/|G| > 1$. *$S_n$ Generators:* Transpositions ${(1 space i) : 2 lt.eq i lt.eq n}$ or ${(2 space 3 dots n), (1 space 2)}$. *$A_n$ Generators:* Elts of $A_n$ ($n gt.eq 3$) product of 3-cycles. Identities (right-to-left): $(i space j)(i space k) = (i space k space j)$ and $(i space j)(k space ell) = (i space j space k)(j space k space ell)$. *$D_6$ Decomposition:* $D_6 tilde.eq D_3 times C_2$. *$A_4$ Subgroups:* No subgroup order 6. *Conjugacy Classes:* \# classes is $1/(|G|) sum_(g in G) |C_G(g)|$.

== Exam Strategies

*Sylow analysis:* $|G| = p^k m$. List divisors of $m$ $equiv 1 (mod p)$. Use element counting to eliminate trivial intersections. *Simple group test:* Index bounds ($|G| | m!$), Sylow uniqueness, union of classes, parity test ($2m$, $m$ odd). *Burnside applications:* List symmetries, compute cycle index, apply formula. *Quotient identification:* 1st Isom Thm via homomorphism. *Ring decomposition:* Central idempotents $e_1, dots, e_k$ sum to $1$ ($sum e_i = 1$) and orthogonal ($e_i e_j = 0$). $R = e_1 R plus.o dots plus.o e_k R$.
